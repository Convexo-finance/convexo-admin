...
import { LPBusinessABI } from '@/lib/contracts/abis';
...
// LP_Business BusinessType enum: Corporation=0, LLC=1, Partnership=2, SoleProprietor=3, Other=4
...
function toBusinessType(companyType: string): number {
  return BUSINESS_TYPE_MAP[companyType] ?? 4;
}
...
export function KYBReviewSystem() {
  ...
  const [selected, setSelected] = useState<KybSubmission | null>(null);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<ListResponse>(`/admin/kyb/submissions?${params}`);
  ...
  const fetchVerifId = useCallback(async (userId: string) => {
    try {
      ...
      const v = res.verifications.find((v) => v.type === 'KYB_BUSINESS' && v.provider === 'INTERNAL');
      if (v) setVerifId(v.id);
    } catch { /* non-fatal */ }
  }, []);

  const selectSubmission = (sub: KybSubmission) => {
    setSelected(sub);
    setReviewNote('');
    setReviewMsg('');
    setRecordMsg('');
  ...
  const handleReview = async (status: 'APPROVED' | 'REJECTED') => {
    ...
    setReviewMsg('');
    try {
      await apiFetch(`/admin/kyb/submissions/${selected.id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status, reviewNote: reviewNote || undefined }),
      });
      const updated: KybSubmission = { ...selected, status, reviewNote: reviewNote || undefined };
      setSelected(updated);
  ...
  const handleMint = () => {
    if (!selected || !contracts?.LP_BUSINESS) return;
    writeContract({
      address: contracts.LP_BUSINESS as `0x${string}`,
      abi: LPBusinessABI,
      functionName: 'safeMint',
      args: [
        selected.user.walletAddress as `0x${string}`,
        selected.companyName,
        selected.incorporationNumber,
        selected.officeCountry,
        toBusinessType(selected.companyType),
        selected.id,
        IPFS.LP_BUSINESS_URI,
      ],
    });
  };
  ...
  const handleDownload = async (doc: KybDoc) => {
    try {
    ...
    <div className="space-y-4">
      <div>
        <h2 className="text-2xl font-bold text-white mb-1 flex items-center gap-3">
          <BuildingOffice2Icon className="w-7 h-7 text-cyan-400" />
          KYB Review — Business Submissions
        </h2>
        <p className="text-gray-400 text-sm">Review business documents and approve/reject KYB for business accounts.</p>
      </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <p className="text-xs text-white font-medium truncate">{sub.companyName}</p>
                        <p className="text-xs font-mono text-gray-500 truncate mt-0.5">{sub.user.walletAddress}</p>
        ...
        <div className="lg:col-span-2">
          {!selected ? (
            <div className="card p-10 text-center">
              <BuildingOffice2Icon className="w-10 h-10 text-gray-600 mx-auto mb-3" />
            ...
            <div className="space-y-4">
              ...
              <div className="card p-4 flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-base font-semibold text-white">{selected.companyName}</p>
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500 pt-1">
                    <span>Type: <span className="text-gray-300">{selected.companyType}</span></span>
                    <span>Reg #: <span className="text-gray-300 font-mono">{selected.incorporationNumber}</span></span>
                    <span>Country: <span className="text-gray-300">{selected.officeCountry}</span></span>
                  </div>
                ...
                <div className="card p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-2 bg-gray-800 border border-gray-700 rounded text-white placeholder-gray-500 text-sm focus:border-cyan-500 focus:outline-none resize-none h-20"
                    />
                  </div>
              ...
              {selected.status === 'APPROVED' && (
                <div className="card p-4 border-cyan-700/30 bg-cyan-900/10 space-y-3">
                  <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                    <BuildingOffice2Icon className="w-4 h-4 text-cyan-400" />
                    Mint LP_Business NFT (Tier 2)
                  </h3>
                  <div className="text-xs text-gray-400 space-y-0.5">
                    <p>Recipient: <span className="font-mono text-white">{selected.user.walletAddress}</span></p>
                    <p>Company: <span className="text-white">{selected.companyName}</span></p>
                    <p>Reg #: <span className="font-mono text-white">{selected.incorporationNumber}</span></p>
                    <p>Jurisdiction: <span className="text-white">{selected.officeCountry}</span></p>
                    <p>Business type: <span className="text-white">{selected.companyType} ({toBusinessType(selected.companyType)})</span></p>
                  ...
                  {!mintHash ? (
                    <button
                      onClick={handleMint}
                      disabled={minting || !contracts?.LP_BUSINESS}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-xs">
                        {mintConfirming
                          ? <><ArrowPathIcon className="w-3.5 h-3.5 text-cyan-400 animate-spin" /><span className="text-cyan-300">Confirming...</span></>
                          : mintSuccess
